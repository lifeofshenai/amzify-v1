...
export default function AIRecommendations({ channels }: AIRecommendationsProps) {
  const getRecommendations = () => {
    const recommendations = [];
    const totalBudget = Object.values(channels).reduce((sum, channel) => sum + channel.budget, 0);

    // Find best performing channel
    const bestChannel = Object.entries(channels).reduce((best, [channel, metrics]) => {
      if (!best || metrics.roas > best.metrics.roas) {
        return { channel, metrics };
      }
      return best;
    }, null as { channel: string; metrics: ChannelMetrics } | null);

    if (bestChannel) {
      const currentAllocation = (bestChannel.metrics.budget / totalBudget) * 100;
