...
export default function CompanyDetailsForm() {
  const { vendorDetails, updateVendorDetails, completeStep, setCurrentStep } = useOnboardingStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!vendorDetails.firstName || !vendorDetails.lastName || !vendorDetails.companyName) {
      ...
      if (isIncorporated) {
        ...
        if (vendorDetails.countryOfIncorporation === 'US') {
          ...
          if (!vendorDetails.companyNumber) {
            toast.error('Company registration number is required for South African companies');
            return;
          }
      ...
      if (isIncorporated) {
        updateVendorDetails({
          currency: vendorDetails.countryOfIncorporation === 'US' ? 'USD' : 'ZAR'
        });
      }
        ...
        <div className="md:col-span-2">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              {showNameGenerator ? 'Hide Name Generator' : 'Need help with a name?'}
            </button>
          </div>
          {showNameGenerator ? (
          ...
          <>
            <div className="md:col-span-2">
              ...
              <select
                ...
                onChange={(e) => updateVendorDetails({
                  countryOfIncorporation: e.target.value as 'US' | 'ZA'
                })}
                className="mt-1 block w-full rounded-lg border border-gray-200 px-4 py-2 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                required
              >
              ...
              <>
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => updateVendorDetails({ companyNumber: e.target.value })}
                    className="mt-1 block w-full rounded-lg border border-gray-200 px-4 py-2 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateVendorDetails({ taxNumber: e.target.value })}
                    className="mt-1 block w-full rounded-lg border border-gray-200 px-4 py-2 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateVendorDetails({ vatNumber: e.target.value })}
                    className="mt-1 block w-full rounded-lg border border-gray-200 px-4 py-2 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  />
                </div>
              </>
