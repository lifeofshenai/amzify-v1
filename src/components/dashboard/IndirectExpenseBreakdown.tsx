...
export default function IndirectExpenseBreakdown({ expenses }: IndirectExpenseBreakdownProps) {
  const totalExpenses = Object.values(expenses).reduce((sum, val) => sum + val, 0);
  const getPercentage = (value: number) => ((value / totalExpenses) * 100).toFixed(1);
